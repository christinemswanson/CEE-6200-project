...
#args = sys.argv # set the parameters of the experiment to use in command line
args = ["", "climate_scenarios", "climate_scenarios", "full", "sq", "1", "0"] # changed input directory name and code version to climate_scenarios

# set working directory
wd = "C:/Users/cms549/Desktop/GitHub/CEE-6200-project/finalProject/simulation" 
os.chdir(wd)
...
# need to change filelist to climate change supply data, not historic! 
#filelist = os.listdir(os.getcwd() + "/input/" + expName + "/*.txt") # point to the CLIMATE CHANGE input file
#filelist = glob.glob(os.path.join(os.getcwd(), "input", expName, "*.txt"))
filelist = ["ssp245_AWI-CM-1-1-MR.txt", "ssp585_MRI-ESM2-0.txt", "ssp585_UKESM1-0-LL.txt"]
for f in range(startf, len(filelist)): # loop through each of the 3 CC water supply files?

    fn = filelist[f].split(".txt")[0] # each of the 3 text files for CC data
    #print("fn is: ", fn)
    os.makedirs(
        "output/" + expName + "/" + season + "/" + str(skill) + "/" + fn, exist_ok=True
    ) # make output folders for each of the 3 CC input data files 
    ...
    for p in range(nseeds):
        ...
        # load input hydrologic data - CHANGE TO CC SUPPLY DATA
        data = pd.read_table("input/" + expName + "/" + filelist[f]) # for each of the 3 files 

        # load short term forecast predictions (CLIMATE CHANGE) 
        sf = pd.read_table("input/" + expName + "/short_forecast/sq/" + filelist[f])

        # load long term forecast predictions (CLIMATE CHANGE)
        lf = pd.read_table(
            "input/"
            + expName
            + "/long_forecast/"
        ...
        for t in range(s, timesteps): # range of the data length df
            ...
            lfSupply = data["forNTS"][t] # forecasted net total supply (supply index)
            lfCon = data["confidence"][t] # confidence indicator 
            lfInd = data["indicator"][t] # long-term trend indicator
            ...
            # flow, level, and flag if september (qm 32) levels are dangerously high (B2.1)
            if qm <= 32:
            ...
            for k in range(nforecasts):
                ...
                # set indicators
                ice = 0
                adj = 0.0014 * (2010 - 1985) # adjustment for differential crustal movement
                epsolon = 0.0001
                ...
                while flg == 1:
                    ...
                    # above average forecasted supplies
                    if lfSupply >= 7011:

                        # set c1 coefficients based on how confident forecast is in wet/extreme supplies
                        if lfInd == 1 and lfCon == 3:
                            c1 = 260
                        else:
                            c1 = 220

                        # rule curve release for above average supplies
                    ...
                    # below average forecasted supplies
                    if lfSupply < 7011:

                        # set c2 coefficient
                        c2 = 60

                        # rule curve release for below average supplies
                    ...
                    # calculate resulting water level from flows
                ...
                # try to keep ontario level up in dry periods (p. 59 below table B1)
                if annavgLevel <= 74.6:

                    # adjust release -> reduce flow by 200 cms 
                    release = release - 20
            ...
            # compute averaged quarter-monthly release (p. 59) 
            ontFlow = round(sum(sfFlow) / nforecasts, 0)
            dif1 = round((sfSupplyNTS[0] / 10 - ontFlow) / conv, 6) # water level conversion
            ...
            elif iceInd == 1 or (qm < 13 or qm > 47): # stable ice or out of nav season
            ...
            if qm >= 13 and qm <= 47:
                ...
                lRegime = "LN" # Navigation season L-limit
            ...
            if lFlow2 < lFlow1:
                lFlow = lFlow2 # replace nav check value from above with channel capacity val if lower
                lRegime = "LC"
            ...
            # historic version of M limit to prevent too low of flows
            elif v == "climate_scenarios": # CMS EDIT TO CLIMATE_SCENARIOS FOR TESTING...used to be "historic"
                ...
                if ontLevel > 74.20:
                    ...
                    if ontLevel >= (adj + 69.474):
                        ...
                        mq2 = np.nan
                    mq = min(mq1, mq2) # take the smaller of the two values
            ...
            if ontFlow >= ontFlowPrev:
                ...
                if flowdif > jup:
                    jlim = ontFlowPrev + jup # force to upper J limit
                    jmaxup = jlim
                    jmin = 0
                    jFlow = jlim
            ...
            # if the flow difference is negative, check if minJ applies
                ...
                if flowdif > jdn:
                    jlim = ontFlowPrev - 70 # force to lower J limit
                    jmaxup = 9999
                    jmin = jlim
                    jFlow = jlim
                    jRegime = "J-"
            ...
            if lLimFlow != 0: # If L limit applies
                if maxLim < 0:
                    maxLim = lLimFlow
                    maxRegime = lRegime

            if iLimFlow != 0: # If I limit applies
                if maxLim < 0 or iLimFlow < maxLim: # If I limit < L limit
                    maxLim = iLimFlow
                    maxRegime = iRegime
            ...
            ) # SIMULATION IS STUCK HERE...THERE ARE NO ROUGHNESS COEFFICIENTS IN THE CLIMATE CHANGE DATA

            # determine "action level" to apply at pointe claire (Table B4)
        ...
        # calculate how long it takes to run the simulation
        sim_et = datetime.now()
        sim_ct = (sim_et - sim_st).total_seconds() / 60
        print(sim_ct)

        # convert the dictionary back to a dataframe for saving
