...
#args = sys.argv
args = ["", "climate_scenarios", "sq"] # 2nd arg is the input directory name 
...
os.makedirs("./input/" + expName + "/short_forecast/" + skill, exist_ok=True) # make short forecast sub-folder in the climate change scenarios folder 
...
# run through CLIMATE CHANGE input hydrologic files
# filelist = os.listdir("../input/" + expName + "/hydro")
path = "./input/" + expName + "/*.txt" # run through each file? CHANGED
filelist = glob(path)
